import { 
  Controller, Post, Body, Get,Patch, UseGuards, UsePipes, ValidationPipe, HttpCode, HttpStatus, 
  Request, Param, Put, Delete, InternalServerErrorException 
} from '@nestjs/common';
...
export class PharmacistController {
  ...
  @UseGuards(AuthGuard)
  async updateProfile(@Request() req, @Body() updatePharmacistDTO: UpdatePharmacistDTO): Promise<any> {
    try {
      return await this.pharmacistService.updateProfile(req.user.email, updatePharmacistDTO);
    } catch (error) {
  ...
  async prescriptionHistory(@Request() req): Promise<any> {
    try {
      ...
      throw new InternalServerErrorException("Failed to retrieve prescription history.");
    }
  }
  ...
  async receiveMedicationAlerts(@Request() req, @Body() medicationAlertDTO: MedicationAlertDTO): Promise<any> {
    try {
      ...
      throw new InternalServerErrorException("Failed to receive medication alerts.");
    }
  }
  ...
  async billingHistory(@Request() req): Promise<any> {
    try {
      ...
      throw new InternalServerErrorException("Failed to retrieve billing history.");
    }
  }
